 ...
 * LLM이 잘못된 필드명을 사용할 경우 대응
 */
function attemptPartialRecovery(response: string): Partial<LLMDialogueResponse> | null {
  try {
    // 먼저 JSON 파싱 시도하여 필드명 변환
    const jsonStr = response.replace(/```(?:json)?\s*([\s\S]*?)```/, '$1').trim();
    try {
      const parsed = JSON.parse(jsonStr);
      // LLM이 잘못된 필드명을 사용한 경우 변환
      ...
      if (content) {
        return {
          ...
          affectionModifier: parsed.affectionModifier || parsed.affection_modifier || 0,
        };
      }
    } catch {
      // JSON 파싱 실패시 정규식으로 추출 시도
    }

    // JSON에서 content 또는 response 필드 추출 시도 (정규식)
